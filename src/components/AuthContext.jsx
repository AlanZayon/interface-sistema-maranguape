...
export const AuthProvider = ({ children }) => {
  ...
  const addFuncionarios = (newData) => {
    setFuncionarios((prevUsers) => {
      ...
      updatedUsersArray.forEach((user) => {
        ...
        const index = prevUsersObject[coordenadoriaId].findIndex(
          (u) => u._id === user._id
        );
  ...
  const addFuncionariosPath = (newData) => {
    setFuncionariosPath((prevUsers) => {
      ...
      for (const key in prevUsers) {
        prevUsersObject[key] = Array.isArray(prevUsers[key])
          ? []
          : { ...prevUsers[key] };
      }
      ...
      const coordenadoriasExistentes = new Set(
  ...
  const checkAuthentication = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/usuarios/verify`, {
